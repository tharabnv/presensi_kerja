<div class="p-8 bg-white shadow rounded">
    ...
    <table class="w-full table-auto border">
        <thead class="bg-gray-100">
            <tr>
                <th class="border px-2 text-center">Nomor</th>
                <th class="border px-2">Nama</th>
                <th class="border px-2 text-center">Nomor</th>
                <th class="border px-2 text-center">Waktu</th>
                <th class="border px-2 text-center">Keterangan</th>
            </tr>
        </thead>
        <tbody>
            @foreach ($presensis as $i => $item)
                <tr>
                    ...
                    <td class="border px-2">{{ $item->nama_pekerja }}</td>
                    <td class="border px-2 text-center">{{ $item->nomor_pekerja }}</td>
                    <td class="border px-2 text-center">
                        {{ \Carbon\Carbon::parse($item->waktu_presensi)->format('d/m/Y H:i') }}
                    </td>
                    <td class="border px-2 text-center">{{ $item->keterangan }}</td>
                </tr>
            @endforeach
        </tbody>
    </table>
